...
int main() {
    ...
    UniversKDtree<2> universKDtree({L1, L2}, cutOffRadius);
    ...
    for (int i = 0; i < N1; ++i) {
        for (int j = 0; j < N1; ++j) {
            ...
            universKDtree.addParticle(particle);
        }
    }
    ...
    // run the simulation
    universKDtree.initUnivers(epsilon, sigma);
    for (double t = 0; t < tEnd; t += dt) {
        universKDtree.update(dt, epsilon, sigma);
        // vtkConverter.createFile();
        // vtkConverter.convertToVTK(univers);
        // vtkConverter.closeFile();
    }
